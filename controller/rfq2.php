        ...
        if (empty($errors)) {
            ...
            if (!$query) {
                ...
                try {
                    ...
                    $detail_sql = "INSERT INTO expense_details (rfq_id, CategoryId, Amount,AmountWithoutTax, Description, salesTax) 
                                VALUES (?, ?, ?, ?, ?, ?)";
                    ...
                    foreach ($expenses as $expense) {
                        ...
                        $detail_stmt->bind_param("isddsi", $inserted_id, $expenseCategory, $totalWithTax,$amount, $description, $tax);
            ...
            try {
                foreach ($expenses as $expense) {
                    ...
                    if ($rfq_id && $ExpenseDetailId) {
                        ...
                        $query2 = "INSERT INTO expense_details (rfq_id, CategoryId, Amount,AmountWithoutTax, Description, salesTax) VALUES (?, ?, ?, ?, ?, ?)";
                        $query2_stmt = $conn->prepare($query2);
                        $query2_stmt->bind_param( "iiddsi", $rfq_id, $expenseCategory, $totalWithTax, $amount, $description, $tax);
                        $query2_stmt->execute();
                    }
...
function getList($conn, &$arr,$data){
    if(isset($data['vendor_id'])){
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $jobStatus = $statusResult['status'];
            if (!in_array(strtolower($jobStatus), array_map('strtolower', ['job satisfaction requested','isApproved','approve', 'approve ny manager', 'submit quotation', 'pending', 'return', 'deactivate', 'return by manager', 'deactivate by manager']))) {
                $satisfaction = true;
            }else{
                $satisfaction = false;
            }
