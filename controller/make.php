...
function view($conn, &$arr){
    $query= $conn->prepare("select * from make");
    $query->execute();
...
function add($conn, &$arr){
    ...
    if (empty($makename)) {
        ...
        $dupCheck = $conn->prepare("select makename from make where makename = ?");
        $dupCheck->bind_param('s', $makename);
        $dupCheck->execute();
        ...
        if ($fetch > 0) {
                $arr["message"] = "Make Name already exists";
        }else{
            $creationdate = date("Y-m-d"); 
            $creationby = $_SESSION['user_id'];
            $query = $conn->prepare("INSERT INTO make(makename,creationdate, creationby) VALUES ( ?, ?, ?)");
            if (!$query) {
                die("Query preparation failed: " . $conn->error); 
            }else{
                $query->bind_param('ssi',$makename, $creationdate, $creationby);
                $exe=$query->execute();
