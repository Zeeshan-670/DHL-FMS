        ...
        if (!empty($isManager['data'])) {  
            foreach ($isManager['data'] as $manager) {
                if ($manager['name'] == $_SESSION['name'] ) {
                    if($_SESSION['max'] >= $rfq['grandTotal']){
                    $authorized = true;
                    $arr['data'] = [];
                    if($status == 'deactivate'){
                        if (empty($reason)) {
...
function higherAuthority($conn,&$arr,$data){
    ...
    $statusResult = checkStatus($conn,$arr,$rfq_id);
    $jobStatus = $statusResult['status'];
    if (!in_array($jobStatus, ['isApproved', 'isApprovedByGops'])) {
        $arr['message'] = "Only approval forwarded status can further be approved by final authority";
        return;
    }
            ...
            foreach ($isGOPS['data'] as $gops) {
                if ($gops['name'] == $_SESSION['name']) {
                    if($_SESSION['max'] >= $rfq['grandTotal']){
                        ...
                        $gopsQuery = $conn->prepare("SELECT users.id, users.name, title, access,email, s.name AS station FROM designation 
                        JOIN users ON designation_id = designation.id 
                        JOIN stationdata s ON s.id = users.station_id 
                        WHERE title LIKE '%GOPS%' and max > ?");
                        if (!$gopsQuery) {
                            die("Query preparation failed: " . $conn->error);
                        }    
                        $gopsQuery->bind_param("i",$_SESSION['max']);
                        $gopsQuery->execute();
                        $result = $gopsQuery->get_result();
                        $gops = [];
...
//isApproved .. approve by manager .. approve .. Pending

function reject($conn,&$arr,$data){
    if($_SESSION['type'] == 'admin' || $_SESSION['type'] == 'User'){
        $creationdate = date("Y-m-d H:i:s");
        $creationby = $_SESSION['user_id'];
        $type = "user";
        ...
        if(in_array($jobStatus, ['isApproved','approve','Pending','submit quotation'])){
            if (empty($reason)) {
            ...
            if (empty($errors)) {
                ...
                $arr['success'] = true;
                $arr['message'] = "RFQ returned successfully";
                $data['data'] = [];
        ...
        if($jobStatus == 'job satisfaction requested'){
            ...
            if (empty($errors)) {
                ...
                if ($exe === false) {
                    ...
                    $query = $conn->prepare("SELECT vendor_id FROM rfq_entries WHERE rfq_id = ?");
                    $query->bind_param('i', $rfq_id);
                    $query->execute();
                    $query->bind_result($vendor_id);
                    $query->fetch();
                    $query->close();

                    $ratingQuery = $conn->prepare("INSERT INTO rating (rfq_id, vendor_id,rating) VALUES (?, ?,?)");
                    if (!$ratingQuery) {
                        die("Query preparation failed: " . $conn->error);
                    }
                    $ratingQuery->bind_param('iii', $rfq_id, $vendor_id,$rating);
                    if (!$ratingQuery->execute()) {
                        die("Execution failed: " . $ratingQuery->error); 
                    }
...
function getList($conn, &$arr,$data){
    if(isset($data['vendor_id'])){
        ...
        left join rating on job.id = rating.id
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $jobs[$job_id]['expenses'][] = [
             ...
             $stmt = $conn->prepare("SELECT 
                                    ...
                                    END AS modifiedby,
                                    CASE WHEN d.`title` IS NULL THEN '-' ELSE d.`title` END  AS designation
...
function getListDhl($conn, &$arr,$data){
    ...
    SELECT 
        ...
        FROM job
        LEFT JOIN vehicledetails vd ON vd.V_id = job.V_id 
        LEFT JOIN invoice i ON i.rfq_id = job.id
        LEFT JOIN rfq_entries r ON r.rfq_id = job.id
        LEFT JOIN users ua ON job.approvedby = ua.id
        LEFT JOIN users uc ON job.checkby = uc.id
        LEFT JOIN users ucb ON r.createdby = ucb.id
        JOIN vehicles v ON v.`V_id` = job.`V_id`
        JOIN stationdata s ON v.station_id = s.id
        JOIN vendor ON vendor.vendorid = job.vendor_id
        LEFT JOIN rating ON rating.rfq_id = r.rfq_id
